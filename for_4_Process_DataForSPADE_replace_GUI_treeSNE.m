function [FlowSPD_output_filename] = for_4_Process_DataForSPD_replace_GUI_treeSNE(treeSNE_parameters)
...
function [loaded_variables] = load_variables(treeSNE_parameters)
	...
	% set save_filename
	if numel(loaded_variables.used_markers)==size(data,1)
		loaded_variables.result_filename = [loaded_variables.data_filename(1:end-4),'_result_TargetGroups',num2str(loaded_variables.target_num_sample_groups),'_Markers_all.mat'];
	else
		tmp = num2str(loaded_variables.used_markers'); tmp(strfind(tmp,'  '))='_'; tmp(tmp==' ')=[];
		loaded_variables.result_filename = [loaded_variables.data_filename(1:end-4),'_result_TargetGroups',num2str(loaded_variables.target_num_sample_groups),'_Markers',tmp,'.mat'];
	end
...
function [loaded_variables] = calc_clusters_and_MST(loaded_variables)
	...
	[mst_adj_dist, sample_group_assign, sample_modules] = ...
		group_flow_samples_to_tree_treeSNE(loaded_variables.data, loaded_variables.used_markers, loaded_variables.extra_markers, loaded_variables.target_num_sample_groups);
